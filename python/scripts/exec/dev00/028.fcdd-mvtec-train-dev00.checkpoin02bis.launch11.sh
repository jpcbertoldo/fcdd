...
sbatch ${SBATCH_SCRIPT_FPATH} ${UNSUPERVISED_ARGS} --loss-mode pixel-level-balanced --net FCDD_CNN224_VGG_512_F
sleep 15  # avoid the same directory name

sbatch ${SBATCH_SCRIPT_FPATH} ${SEMI_SUPERVISED_ARGS} --loss-mode pixel-level-balanced --net FCDD_CNN224_VGG_512_F
sleep 15  # avoid the same directory name
