...
class UNETDDTrainer(BaseTrainer):
    """
    based on BaseADTrainer
    """
    ...
    def loss(self, outs: Tensor, ins: Tensor, labels: Tensor, gtmaps: Tensor = None):
        ...
        # this part is already in the model
        # loss = outs ** 2
        # loss = (loss + 1).sqrt() - 1

        ############## CONTINUE HERE 
        # COMPLETE THE classs by doing the other methods
        # and test this one
        ...
        if self.net.training:
            ...
            if exclude_complete_nominal_samples.sum() > 0:
                ...
                a = a.view(a.size(0), -1).mean(-1)
                anom[exclude_complete_nominal_samples] = -(((1 - (-a).exp()) + 1e-31).log())
