...
import wandb
from callbacks_dev01 import (HEATMAP_NORMALIZATION_PERCENTILES_IN_EPOCH, LOG_HISTOGRAM_MODES,
...
def run_one(
    ...
    def log_model_architecture(model_: torch.nn.Module):
        ...
        wandb.save(str(model_str_fpath), policy="now", base_path=str(logdir)) 
...
parser.set_defaults(
    ...
    scheduler_parameters=[0.9970],
    weight_decay=1e-4,
    batch_size=6,
    lightning_accumulate_grad_batches=1,  # half epoch for class 0
    # classes=[3],
    ...
    wandb_tags=["dev01", "u2net", "sweep-02"],
    wandb_profile=False,
    wandb_offline=False,
    wandb_watch=None,
