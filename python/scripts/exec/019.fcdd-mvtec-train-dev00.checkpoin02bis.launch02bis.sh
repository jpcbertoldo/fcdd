...
sbatch ${SBATCH_SCRIPT_FPATH} ${SEMI_SUPERVISED_ARGS} --loss-mode pixel-level-balanced
sleep 7  # avoid the same directory name


sbatch ${SBATCH_SCRIPT_FPATH} ${SEMI_SUPERVISED_ARGS} --loss-mode pixel-wise-average-distance-per-image
sleep 7  # avoid the same directory name


sbatch ${SBATCH_SCRIPT_FPATH} ${SEMI_SUPERVISED_ARGS} --loss-mode pixel-wise-average-distances
sleep 7  # avoid the same directory name
