...
sbatch ${SBATCH_SCRIPT_FPATH} ${SEMI_SUPERVISED_ARGS} --loss-mode pixel-level-balanced-post-hubert
sleep 15  # avoid the same directory name


sbatch ${SBATCH_SCRIPT_FPATH} ${SEMI_SUPERVISED_ARGS} --loss-mode pixel-level-balanced-hubert-factor-average
sleep 15  # avoid the same directory name


sbatch ${SBATCH_SCRIPT_FPATH} ${UNSUPERVISED_ARGS} --loss-mode pixel-level-balanced-post-hubert
sleep 15  # avoid the same directory name


sbatch ${SBATCH_SCRIPT_FPATH} ${UNSUPERVISED_ARGS} --loss-mode pixel-level-balanced-hubert-factor-average
sleep 15  # avoid the same directory name
