...
class BatchRandomFlip(BatchTransformMixin, RandomTransformMixin):
    """Vertically and horizontally flip the images in a batch with the respective given probabilities.
    The image is torch Tensor and expected to have [..., H, W] shapes.
    """
    
    @RandomTransformMixin._init_torch_generator
    def __init__(self, vertical_probability: float, horizontal_probability: float):
        
        def validate_and_warn_probability(p, name):
            assert 0 <= p <= 1, f"probability must be between 0 and 1, got {p}"
            if p == 0:
                warnings.warn(f"probability of {name} flip is 0, no flipping will be applied", stacklevel=2)
            if p == 1:
                warnings.warn(f"probability of {name} flip is 1, flipping will be always applied", stacklevel=2)

        validate_and_warn_probability(vertical_probability, "vertical")
        validate_and_warn_probability(horizontal_probability, "horizontal")
        
        self.vertical_probability = vertical_probability
        self.horizontal_probability = horizontal_probability

    @staticmethod
    def get_params(batch_size: int, vertical_probability: float, horizontal_probability: float, torch_generator: torch.Generator) -> Tensor:
        probas = torch.rand((batch_size, 2), generator=torch_generator)
        # dim 1: (vertical, horizontal)
        flags = torch.empty_like(probas, dtype=torch.bool)
        flags[:, 0] = probas[:, 0] < vertical_probability
        flags[:, 1] = probas[:, 1] < horizontal_probability
        return flags  # booleans
    
    @staticmethod
    def flip(flip_flags: Tensor, batch: Tensor) -> Tensor:
        # vertical flip: dim -2
        # horizontal flip: dim -1
        batch[flip_flags[:, 0]] = batch[flip_flags[:, 0]].flip(-2)
        batch[flip_flags[:, 1]] = batch[flip_flags[:, 1]].flip(-1)
        return batch

    @BatchTransformMixin._validate_before_and_after_is_same()
    def __call__(self, batch: Tensor) -> Tensor:
        # flip_flags \in {False, True}^(batchsize, 2)
        # dim 1: (vertical, horizontal)
        batch_size = batch.shape[0]
        flip_flags = BatchRandomFlip.get_params(batch_size, self.vertical_probability, self.horizontal_probability, self.torch_generator)
        return self.flip(flip_flags, batch)         
    
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(vertical_probability={self.vertical_probability:.4f}, horizontal_probability={self.horizontal_probability:.4f})"


class LoopBatchRandomAffine(BatchTransformMixin, RandomTransformMixin):
    """
    The batch of images is a torch Tensor expected to have [..., H, W] shape.
    
    IMPORTANT: 'fill' value is always 0!
    """
    ...
    def __init__(
        ...
        shear_degrees: Tuple[float, float],  # vertical, horizontal
        interpolation=T.InterpolationMode.NEAREST,
    ):
        
        T._check_sequence_input(scale_range, "scale_range", req_sizes=(2,))
        
        for s in scale_range:
            assert s > 0, f"scale_range must be positive, got {s}"
            if s == 1:
                warnings.warn(f"scale_range={scale_range} contains 1, which means no scaling", stacklevel=2)
        
        assert 0 <= rotation_degrees <= 180, f"rotation_degrees must be between 0 and 180, got {rotation_degrees}"
        if rotation_degrees == 0:
            warnings.warn(f"rotation_degrees={rotation_degrees} is 0, no rotation will be applied", stacklevel=2)
        
        self.scale_range = scale_range
        self.rotation_degrees = rotation_degrees
        self.shear_degrees = T._setup_angle(shear_degrees, name="shear_degrees", req_sizes=(2,))
    ...
    def get_params(
        ...
        rotation_degrees: float,
        scale_range: Tuple[float, float],
        shear_degrees: Tuple[float, float],
        torch_generator: torch.Generator,
    ) -> Tensor:
        """Get parameters for affine transformation
        Returns:
            tensor of shape (batch_size, 4)
            dim 1: (rotation, scale, shear_vertical, shear_horizontal)
            angles in degrees
        """
        rotation_angles_degrees = torch.empty((batch_size, 1)).uniform_(-rotation_degrees, rotation_degrees, generator=torch_generator)

        scale_min, scale_max = scale_range
        scales = torch.empty((batch_size, 1)).uniform_(scale_min, scale_max, generator=torch_generator)

        shear_vertical, shear_horizontal = shear_degrees
        rotation_angles_degrees_vertical = torch.empty((batch_size, 1)).uniform_(-shear_vertical, shear_vertical, generator=torch_generator)
        rotation_angles_degrees_horizontal = torch.empty((batch_size, 1)).uniform_(-shear_horizontal, shear_horizontal, generator=torch_generator)
        
        return torch.stack([rotation_angles_degrees, scales, rotation_angles_degrees_vertical, rotation_angles_degrees_horizontal], dim=1)

    @staticmethod
    def apply_affine(params: Tensor, batch: Tensor, interpolation) -> Tensor:
        
        nchannels = batch.shape[1]
        fill = [LoopBatchRandomAffine.FILL_VALUE] * nchannels
        
        batch_size = batch.shape[0]
        
        # params is (batch_size, 4)
        # dim 1: (rotation, scale, shear_vertical, shear_horizontal)
        for idx in range(batch_size):
            batch[idx] = TF.affine(
                img=batch[idx], 
                angle=params[idx, 0].item(),
                scale=params[idx, 1].item(),
                shear=params[idx, 2:].view(-1).tolist(),
    ...
    @BatchTransformMixin._validate_before_and_after_is_same()
    def __call__(self, batch: Tensor) -> Tensor:
        # params \in float^(batch_size, 4)
        # dim 1: (rotation, scale, shear_vertical, shear_horizontal)
        # angles in degrees
        batch_size = batch.shape[0]
        params = LoopBatchRandomAffine.get_params(batch_size, self.rotation_degrees, self.scale_range, self.shear_degrees, self.torch_generator)
        return self.apply_affine(params, batch, self.interpolation)

    def __repr__(self) -> str:
        s = f"{self.__class__.__name__}(rotation_degrees={self.rotation_degrees}"
        s += f", scale_range={self.scale_range}"
        s += f", shear_degrees={self.shear_degrees}" 
        s += f", interpolation={self.interpolation.value}" if self.interpolation != T.InterpolationMode.NEAREST else ""
        s += f", fill={self.fill}"
        s += ")"
        return s
...
def make_multibatch_use_same_random_state(transform: RandomTransformMixin):
    ...
    assert isinstance(transform, RandomTransformMixin), f"transform must be a {RandomTransformMixin.__name__}, got {type(transform)}"
    
    print("make_multibatch_use_same_random_state: {transform.__class__.__name__}")
    
    def decorate_numpy(__call__: Callable[[Tensor], Tensor]):
        
        generator: np.random.Generator = transform.generator
    ...
    def decorate_torch(__call__: Callable[[Tensor], Tensor]):
        
        torch_generator: torch.Generator = transform.torch_generator
        
        @functools.wraps(__call__)
        def wrapper(*batches: List[Tensor], **kwargs) -> List[Tensor]:
            initial_state = torch_generator.get_state()
            returns = []
            for batch in batches:
                torch_generator.set_state(initial_state)
                ret = __call__(batch, **kwargs)
                returns.append(ret)
                # by setting the random state at the beginning, the state will still have
                # advanced as this loop finishes the last iteration
    ...
    if hasattr(transform, "generator"):
        print(f"make_multibatch_use_same_random_state: {transform.__class__.__name__} has generator (numpy), decorating with {decorate_numpy.__name__}")
        decorate = decorate_numpy    
    
    elif hasattr(transform, "torch_generator"):
        print(f"make_multibatch_use_same_random_state: {transform.__class__.__name__} has torch_generator (torch), decorating with {decorate_torch.__name__}")
        decorate = decorate_torch
        
    else: 
        raise ValueError(f"transform must have generator or torch_generator, but got {transform.__class__.__name__}")
    
    if isinstance(transform, torch.nn.Module):
        print(f"make_multibatch_use_same_random_state: {transform.__class__.__name__} IS torch.nn.Module, so decorating transform.forward")
        transform.forward = decorate(transform.forward)

    else:
        print(f"make_multibatch_use_same_random_state: {transform.__class__.__name__} is NOT torch.nn.Module, so decorating transform (implicitely __call__)")
        transform = decorate(transform)
