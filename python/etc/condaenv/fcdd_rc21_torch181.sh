...
# second attempt
# retyring with the pip line modified

mv requirements.txt requirements.txt.hold
mv etc/condaenv/requirements-no-torch.txt requirements.txt

pip install --editable .

# src: https://stackoverflow.com/a/66833948/9582881
pip install --force-reinstall torch==1.8.1+cu101 torchvision==0.9.1+cu101 -f https://download.pytorch.org/whl/torch_stable.html

mv requirements.txt etc/condaenv/requirements-no-torch.txt
mv requirements.txt.hold requirements.txt

# THIS ONE WORKED!!!!
# src: https://stackoverflow.com/a/66833948/9582881
# other modif: when installing fcdd i removed torch and torchvision from the dependencies. 
